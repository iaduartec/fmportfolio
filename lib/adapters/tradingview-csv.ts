...
function parseTimestamp(value: string): Date {
  ...
  if (!trimmed) {
    throw new TradingViewCsvError('Timestamp vacio en CSV de TradingView');
  }
  if (/^\d+$/.test(trimmed)) {
    ...
    if (Number.isNaN(date.getTime())) {
      throw new TradingViewCsvError(`No se pudo interpretar la marca de tiempo numerica: ${trimmed}`);
    }
    return date;
  }
  const isoCandidate =
    trimmed.includes(' ') && !trimmed.endsWith('Z') ? `${trimmed.replace(' ', 'T')}Z` : trimmed;
...
export function parseTradingViewCsv(input: string): TradingViewCandle[] {
  try {
    ...
    if (error instanceof z.ZodError) {
      throw new TradingViewCsvError('Formato CSV de TradingView invalido');
    }
